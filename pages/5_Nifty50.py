...
import google.generativeai as genai # Assuming you've set up genai for your LLM calls
...
# Ensure your API key is configured in Streamlit secrets (e.g., in .streamlit/secrets.toml)
# genai.configure(api_key=st.secrets["gemini"]["api_key"]) # Uncomment and configure if using Gemini
...
def get_ai_review(page_name, page_data):
    ...
    for key, value in page_data.items():
        if key not in ["status", "chart_status", "num_data_points", "num_filings", "company", "ticker", "cik", "manager_name", "manager_cik"]:
    ...
    try:
        ...
        if page_name == "Nifty 50 Chart":
        ...
        elif page_name == "Market Trend Visualization": # For the Market Trends page
            ...
            ai_text = (f"The market data for **{ticker}** covers **{num_points}** data points from {page_data.get('date_range')}. "
                       f"The close price ranged from {page_data.get('data_summary', {}).get('min_close', 'N/A'):.2f} to {page_data.get('data_summary', {}).get('max_close', 'N/A'):.2f}, "
                       f"starting at {page_data.get('data_summary', {}).get('first_close', 'N/A'):.2f} and ending at {page_data.get('data_summary', {}).get('last_close', 'N/A'):.2f}. "
        ...
        st.error(f"Error calling LLM for review: {e}")
        return "AI review generation failed due to an internal error."
        ...
        with st.spinner("Fetching Nifty 50 data..."):
            try:
                ...
                if data.empty:
                    ...
                    # --- Step 1: Flatten Multi-level Columns if they exist ---
                    if isinstance(data.columns, pd.MultiIndex):
                        data.columns = ['_'.join(col).strip() for col in data.columns.values]
                        
                        ticker_variable = NIFTY_TICKER # Use the actual ticker variable for this page

                        column_mapping = {}
                        standard_cols = ['Open', 'High', 'Low', 'Close', 'Volume']
                        for std_col in standard_cols:
                            expected_flattened_name = f'{std_col}_{ticker_variable}'.replace('^', '') # Remove ^ if present
                    ...
                    # --- Step 2: Ensure critical columns are numeric and handle missing values ---
                    for col_name in ['Open', 'High', 'Low', 'Close', 'Volume']:
                        if col_name in data.columns:
                            data[col_name] = pd.to_numeric(data[col_name], errors='coerce')
                        else:
                            st.error(f"Error: Required column '{col_name}' not found after data processing. Cannot plot.")
                            st.stop() # Stop execution if a critical column is missing

                    # --- Step 3: Drop rows with NaN values in crucial plotting columns ---
