...
st.title("📊 Market Trends Data (Raw Data & Chart)")
st.markdown("<p style='font-size: 1.1rem;'>View raw historical data and a candlestick chart for Nifty 50 or other stock/index symbols.</p>", unsafe_allow_html=True)
        ...
        with st.spinner(f"Fetching historical data for {market_ticker}..."):
            try:
                ...
                if not data.empty:
                    ...
                    fig.update_layout(
                        title=f'{market_ticker} Price Chart',
                    ...
                    st.info("No data to plot a candlestick chart.")
