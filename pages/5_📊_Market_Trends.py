...
market_ticker = st.text_input(
    ...
    key="mt_market_ticker_input" # Unique key
).strip().upper()
...
with col1:
    chart_start_date = st.date_input("Start Date", value=start_date, key="mt_chart_start_date") # Unique key
with col2:
    chart_end_date = st.date_input("End Date", value=end_date, key="mt_chart_end_date") # Unique key

if st.button("Get Market Data", key="mt_get_market_data_btn"): # Unique key
    if market_ticker:
